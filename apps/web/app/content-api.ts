...
export type PracticeSet = PracticeSetSummary & {
...
export type PracticeSetForPractice = PracticeSetSummary & {
...
export async function currentUser() {
  return (await api<{ user: AuthUser }>("/auth/session")).user;
}
...
export async function saveContent(
  request: Request,
  resource: "topics" | "materials" | "questions",
  topicId?: string,
) {
  ...
  try {
        ...
        : JSON.stringify(
            resource === "questions"
              ...
              : {
                  ...
                  ...(resource === "materials"
                    ? { content: form.get("content"), topicId: Number(topicId) }
                    : {}),
                },
          ),
    });
    return {
      ok: true,
      error: "",
      intent: deleting ? "delete" : id ? "update" : "create",
    };
  } catch (error) {
    return {
      ok: false,
...
export async function savePracticeSet(request: Request) {
  ...
  try {
    await api(
      `/practice-sets${id ? `/${encodeURIComponent(String(id))}` : ""}`,
      {
        method: deleting ? "DELETE" : id ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: deleting

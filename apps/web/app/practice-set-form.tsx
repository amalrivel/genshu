...
export function PracticeSetForm({ questions, record }: Props) {
  ...
  useEffect(() => {
    ...
    if (fetcher.data?.ok && fetcher.data.intent === "delete" && record)
  ...
  return (
    <fetcher.Form
      method="post"
      ref={form}
      className="content-form practice-set-form"
    >
      {record && <input type="hidden" name="id" value={record.id} />}
      {selectedIds.map((id) => (
        <input key={id} type="hidden" name="questionId" value={id} />
      ))}
      <fieldset disabled={busy}>
        ...
        <h3>Question bank</h3>
        {!questions.length && (
          <p>Create questions before selecting them for a practice set.</p>
        )}
        <ul className="question-selector">
          {questions.map((question) => (
            <li key={question.id}>
              <label>
                ...
                />
                <span lang="ja">{question.japaneseText}</span>
                <span lang="id">{question.indonesianTranslation}</span>
              </label>
            </li>
          ))}
        </ul>
        ...
        <ol className="selected-questions">

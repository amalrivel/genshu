...
export async function clientAction({ request }: Route.ClientActionArgs) {
  ...
  try {
    if (intent === "create")
      return {
        ok: true,
        created: await api<AuthUser>("/admin/users", {
          method: "POST",
    ...
    return {
      ok: true,
      result: await api<LinkResult>(
        `/admin/users/${id}/${intent === "invite" ? "invitations" : "password-resets"}`,
        { method: "POST" },
      ),
    };
  } catch (error) {
  ...
  return (
    <main className="content-admin">
      ...
      <Form method="post" className="content-form">
        <fieldset>
          <legend>Create participant</legend>
          <label>
            Email
      ...
      {data?.created && (
        <p role="status">
          Created {data.created.email}. Generate an invitation below.
        </p>
      )}
      {data?.result && (
        <section className="content-form">
          <h2>Manual message</h2>
          <p>Expires: {data.result.expiresAt}</p>
          <label>
            URL
            <input readOnly value={data.result.url} />
          </label>
          <label>
            Message
            <textarea readOnly rows={5} value={data.result.message} />
          </label>
        </section>
      )}
          ...
          .map((user) => (
            <li key={user.id}>
              ...
              <Form method="post" className="actions">
                ...
                <button name="intent" value="invite">
                  Generate invitation
                </button>
                <button name="intent" value="reset">
                  Generate reset link
                </button>
              </Form>
            </li>

...
import { api, requireAdmin, savePracticeSet, type PracticeSet, type Question } from "../content-api";
...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  const [practiceSet, questions] = await Promise.all([
    api<PracticeSet>(`/practice-sets/${id}`), api<Question[]>("/questions"),
  ]);
...
export function HydrateFallback() { return <main className="content-admin"><p role="status">Loading practice set…</p></main>; }

export default function PracticeSetPage({ loaderData }: Route.ComponentProps) {
  return <main className="content-admin">
    <nav><Link to="/practice-sets">← All practice sets</Link></nav>
    <h1>{loaderData.practiceSet.title}</h1>
    <PracticeSetForm key={loaderData.practiceSet.id} record={loaderData.practiceSet} questions={loaderData.questions} />
  </main>;
}

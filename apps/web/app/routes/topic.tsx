...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  const [topic, materials, questions] = await Promise.all([
    api<Topic>(`/topics/${id}`),
    api<Material[]>(`/materials?topicId=${id}`),
    api<Question[]>(`/questions?topicId=${id}`),
  ]);
...
export async function clientAction({
  ...
  return saveContent(
    request,
    form.get("resource") === "questions" ? "questions" : "materials",
    params.topicId,
  );
}
...
export default function TopicPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <main className="content-admin">
      ...
      <h1>{topic.title}</h1>
      <p>
        Materials and questions are listed alphabetically. Content is saved as
        Markdown or plain text.
      </p>
      <ContentForm key={topic.id} kind="material" />
      <h2>Materials</h2>
      {!materials.length && (
        <p>No materials yet. Create the first material above.</p>
      )}
      ...
      <QuestionForm />
      {!questions.length && (
        <p>No questions yet. Create the first question above.</p>
      )}
      <ul className="content-list">
        {questions.map((question) => (
          <li key={question.id}>
            ...
            <p>
              <strong>
                {question.correctAnswer ? "○ Maru / True" : "× Batsu / False"}
              </strong>
            </p>

...
export default function PracticeSets({ loaderData }: Route.ComponentProps) {
  return (
    <main className="content-admin">
      ...
      {!loaderData.practiceSets.length && (
        <p>No practice sets yet. Create the first set above.</p>
      )}
      <ul className="content-list">
        {loaderData.practiceSets.map((practiceSet) => (
          <li key={practiceSet.id}>
            <h3>
              <Link to={`/practice-sets/${practiceSet.id}`}>
                {practiceSet.title}
              </Link>
            </h3>

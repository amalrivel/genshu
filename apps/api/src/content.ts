...
function submittedAnswer(body: unknown) {
  if (
    ...
    typeof body !== "object" ||
    Array.isArray(body) ||
    typeof (body as Record<string, unknown>).answer !== "boolean"
  ) {
...
participantPractice.post(
  "/practice-sets/:id/questions/:questionId/check-answer",
  async (req, res) => {
    ...
    const link = await db.orm.public.PracticeSetQuestion.where({
      practiceSetId,
      questionId,
    }).first();
    if (!link) {
      res
        .status(404)
        .json({ error: "Question not found in this practice set." });
      return;
    }
    const question = await db.orm.public.Question.where({
      id: questionId,

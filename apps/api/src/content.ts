...
function questionFields(body: unknown) {
  ...
  if (typeof data.correctAnswer !== "boolean")
    invalid("Correct answer must be a boolean.");
  return {
    japaneseText: questionText(data, "japaneseText", "Japanese text") as string,
    indonesianTranslation: questionText(
      data,
    ...
    japaneseExplanation: questionText(
      data,
      "japaneseExplanation",
      "Japanese explanation",
    ) as string,
    indonesianExplanation: questionText(
      data,
...
function questionIds(value: unknown, required = false) {
  ...
  if (!Array.isArray(value))
    invalid(
      "Question IDs must be an array of unique positive PostgreSQL integers.",
    );
  const ids = value.map(id);
  if (new Set(ids).size !== ids.length)
    invalid("Question IDs must not contain duplicates.");
  return ids;
}

async function validateQuestionIds(ids: number[]) {
  if (!ids.length) return;
  const questions = await db.orm.public.Question.where((q) =>
    q.id.in(ids),
  ).all();
  if (questions.length !== ids.length)
    invalid("One or more selected questions do not exist.");
}
...
async function practiceSetDetail(practiceSet: { id: number }) {
  ...
  const questions = links.length
    ? await db.orm.public.Question.where((q) =>
        q.id.in(links.map((link) => link.questionId)),
      ).all()
    : [];
...
function submittedAnswers(body: unknown) {
  ...
  if (!Array.isArray(answers)) invalid("Answers must be an array.");
  const selected = answers.map((item) => {
    if (!item || typeof item !== "object" || Array.isArray(item))
      invalid("Each answer must contain a question ID and boolean answer.");
    const answer = item as Record<string, unknown>;
    if (typeof answer.answer !== "boolean")
      invalid("Each answer must contain a question ID and boolean answer.");
    return { questionId: id(answer.questionId), answer: answer.answer };
  });
  if (new Set(selected.map((answer) => answer.questionId)).size !== selected.length)
    invalid("Answers must not contain duplicate question IDs.");
  return selected;
}
...
content.put("/topics/:id", async (req, res) => {
  ...
  const { title } = fields(req.body);
  const row = await db.orm.public.Topic.where({ id: topicId }).update({
    title,
  });
  if (!row) {
    res.status(404).json({ error: "Topic not found." });
...
content.delete("/topics/:id", async (req, res) => {
  const row = await db.orm.public.Topic.where({
    id: id(req.params.id),
  }).delete();
  if (!row) {
    res.status(404).json({ error: "Topic not found." });
...
content.get("/materials", async (req, res) => {
  ...
  res.json(
    await materials.orderBy([(m) => m.title.asc(), (m) => m.id.asc()]).all(),
  );
});
...
content.delete("/materials/:id", async (req, res) => {
  const row = await db.orm.public.Material.where({
    id: id(req.params.id),
  }).delete();
  if (!row) {
    res.status(404).json({ error: "Material not found." });
...
content.get("/questions", async (req, res) => {
  ...
  res.json(
    await questions
      .orderBy([(q) => q.japaneseText.asc(), (q) => q.id.asc()])
      .all(),
  );
});
...
content.delete("/questions/:id", async (req, res) => {
  const row = await db.orm.public.Question.where({
    id: id(req.params.id),
  }).delete();
  if (!row) {
    res.status(404).json({ error: "Question not found." });
...
participantPractice.get("/practice-sets/:id/practice", async (req, res) => {
  const practiceSet = await db.orm.public.PracticeSet.where({
    id: id(req.params.id),
  }).first();
  if (!practiceSet) {
...
participantPractice.post("/practice-sets/:id/submit", async (req, res) => {
  ...
  if (answers.some((answer) => !questionIds.has(answer.questionId))) {
    invalid("One or more answers do not belong to this practice set.");
  }
  const questions = links.length
    ? await db.orm.public.Question.where((question) =>
        question.id.in(links.map((link) => link.questionId)),
      ).all()
    : [];
  const byId = new Map(questions.map((question) => [question.id, question]));
...
content.post("/practice-sets", async (req, res) => {
  ...
  const practiceSet = await db.transaction(async (tx) => {
    const set = await tx.orm.public.PracticeSet.create({
      title: data.title,
  ...
  res
    .location(`/practice-sets/${practiceSet.id}`)
    .status(201)
    .json(await practiceSetDetail(practiceSet));
});
content.put("/practice-sets/:id", async (req, res) => {
  ...
  const practiceSet = await db.transaction(async (tx) => {
    const set = await tx.orm.public.PracticeSet.where({
      id: practiceSetId,
    }).update({
      title: data.title,
      description: data.description,
...
content.put("/practice-sets/:id/questions", async (req, res) => {
  ...
  const selected = questionIds(
    (req.body as Record<string, unknown>)?.questionIds,
    true,
  )!;
  await validateQuestionIds(selected);
  const practiceSet = await db.transaction(async (tx) => {
...
export const contentError: ErrorRequestHandler = (error, req, res, _next) => {
  ...
  if (error.type === "entity.too.large") {
    res
      .status(413)
      .json({ error: "Content is too large (maximum request size: 1 MB)." });
    return;
  }

...
if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || !password || password.length < 12) {
  throw new Error('Set ADMIN_EMAIL and an ADMIN_PASSWORD of at least 12 characters. ADMIN_NAME is optional.');
}
...
if (existing) {
  ...
  await db.orm.public.User.create({ email, name, role: 'Admin', isActive: true, passwordHash: await hashPassword(password) });

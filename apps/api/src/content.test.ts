...
test("Topic → Material CRUD, validation, filtering and safe deletion", async () => {
  ...
  try {
    ...
    assert.ok(
      allTopics.findIndex((t: { id: number }) => t.id === other.id) <
        allTopics.findIndex((t: { id: number }) => t.id === topic.id),
    );
    assert.equal(
      (
        await request(`/topics/${topic.id}`, "PUT", {
          title: `${title} Updated`,
        })
      ).title,
      `${title} Updated`,
    ...
    const material = await request(
      "/materials",
      "POST",
      { title: "Z signs", content: markdown, topicId: topic.id },
      201,
    );
    ...
    const second = await request(
      "/materials",
      "POST",
      { title: "A signs", content: "Plain text", topicId: topic.id },
      201,
    );
    materials.push(second.id);
    assert.equal(
      (await request(`/materials/${material.id}`)).content,
      markdown,
    );
    assert.deepEqual(
      (await request(`/materials?topicId=${topic.id}`)).map(
        (m: { id: number }) => m.id,
      ),
    ...
    assert.ok(
      (await request("/materials")).some(
        (m: { id: number }) => m.id === material.id,
      ),
    );
    ...
    await request(
      "/materials",
      "POST",
      { title: "Bad", content: "", topicId: topic.id },
      400,
    );
    await request(
      "/materials",
      "POST",
      { title: "Bad", content: "Text", topicId: 2147483647 },
      409,
    );
    const updated = await request(`/materials/${material.id}`, "PUT", {
      title: "Edited",
      content: "## Updated",
    });
    ...
    await request(
      ...
      "PUT",
      { title: "Edited", content: "## Updated", topicId: 2147483647 },
      409,
    );
    await request(`/materials/${material.id}`, "PUT", {
      title: "Edited",
      content: "## Updated",
      topicId: other.id,
    });
    assert.equal(
      (await request(`/materials?topicId=${other.id}`))[0].id,
      material.id,
    );
    const questionData = {
    ...
    assert.equal(
      (await request(`/questions/${question.id}`)).correctAnswer,
      false,
    );
    assert.deepEqual(
      (await request(`/questions?topicId=${topic.id}`)).map(
        (q: { id: number }) => q.id,
      ),
    ...
    await request(`/questions/${question.id}`, "PUT", {
      ...questionData,
      topicId: other.id,
    });
    assert.equal(
      (await request(`/questions?topicId=${other.id}`))[0].id,
      question.id,
    );
    ...
    const malformed = await raw("/topics", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{",
    });
    ...
    for (const id of materials)
      await raw(`/materials/${id}`, { method: "DELETE" });
    for (const id of questions)
      await raw(`/questions/${id}`, { method: "DELETE" });
  ...
  try {
    ...
    for (const suffix of ["A", "B", "C"]) {
      const question = await request(
        ...
        {
          ...questionData,
          japaneseText: `${suffix} ${questionData.japaneseText}`,
        },
        201,
      );
      questions.push(question.id);
    ...
    for (const bad of [
      ...
      { title: " " },
      { title: "bad\0title" },
      { title, questionIds: "bad" },
      { title, questionIds: [firstQuestion, firstQuestion] },
      { title, questionIds: [2147483647] },
      { title, questionIds: [0] },
    ])
    ...
    const practiceSet = await request(
      ...
      "POST",
      {
        ...
        questionIds: [secondQuestion, firstQuestion],
      },
      201,
    );
    ...
    assert.deepEqual(
      practiceSet.questions.map((question: { id: number }) => question.id),
    ...
    assert.deepEqual(
      (await request("/practice-sets"))
        .filter((set: { id: number }) => practiceSets.includes(set.id))
        .map((set: { id: number }) => set.id),
      [otherSet.id, practiceSet.id],
    );
    assert.deepEqual(
      (await request(`/practice-sets/${practiceSet.id}`)).questions.map(
    ...
    const updated = await request(`/practice-sets/${practiceSet.id}`, "PUT", {
      title: `${title} Updated`,
      description: null,
      questionIds: [thirdQuestion, firstQuestion, secondQuestion],
    });
    assert.equal(updated.description, null);
    assert.deepEqual(
      updated.questions.map((question: { id: number }) => question.id),
      [thirdQuestion, firstQuestion, secondQuestion],
    );
    const reordered = await request(
      `/practice-sets/${practiceSet.id}/questions`,
      "PUT",
      { questionIds: [firstQuestion, thirdQuestion] },
    );
    assert.deepEqual(
      reordered.questions.map((question: { id: number }) => question.id),
      [firstQuestion, thirdQuestion],
    );
    for (const bad of [
      { questionIds: [firstQuestion, firstQuestion] },
      { questionIds: [2147483647] },
      { questionIds: [firstQuestion, "bad"] },
    ])
      await request(
        `/practice-sets/${practiceSet.id}/questions`,
        "PUT",
    ...
    await request(
      "/practice-sets/2147483647",
      "PUT",
      { title, questionIds: [] },
      404,
    );
    await request(
      "/practice-sets/2147483647/questions",
      "PUT",
      { questionIds: [] },
      404,
    );
    ...
    for (const id of practiceSets)
      await raw(`/practice-sets/${id}`, { method: "DELETE" });
    for (const id of questions)
      await raw(`/questions/${id}`, { method: "DELETE" });
    for (const id of topics) await raw(`/topics/${id}`, { method: "DELETE" });
  }
...
test("Participant practice payload hides answers and scores final submissions", async () => {
  ...
  try {
    ...
    assert.equal(
      JSON.stringify(practice).includes("japaneseExplanation"),
      false,
    );
    assert.equal(
      JSON.stringify(practice).includes("indonesianExplanation"),
      false,
    );
...
test("Authentication, invitation, reset, session revocation and authorization", async () => {
  ...
  try {
    await publicRequest(
      "/auth/register",
      "POST",
      { email: "public@example.com", password: "Test password 123!" },
      401,
    );
    await publicRequest(
      "/auth/login",
      "POST",
      { email: "unknown@example.com", password: "Test password 123!" },
      401,
    );
    const inactive = await request(
      "/admin/users",
      "POST",
      { email: `inactive-${Date.now()}@example.com`, name: "Inactive" },
      201,
    );
    ...
    await publicRequest(
      "/auth/login",
      "POST",
      { email: inactive.email, password: "Test password 123!" },
      401,
    );

    const firstInvite = await request(
      `/admin/users/${inactive.id}/invitations`,
      "POST",
    );
    assert.match(firstInvite.url, /^http:\/\/localhost:5173\/invite\//);
    assert.ok(
      Math.abs(
        new Date(firstInvite.expiresAt).getTime() -
          (Date.now() + 72 * 60 * 60 * 1000),
      ) < 10_000,
    );
    const secondInvite = await request(
      `/admin/users/${inactive.id}/invitations`,
      "POST",
    );
    ...
    assert.equal(
      (await publicRequest(`/auth/invitations/${firstToken}`)).data.valid,
      false,
    );
    assert.equal(
      (await publicRequest(`/auth/invitations/${secondToken}`)).data.valid,
      true,
    );
    const invitation = (
      await db.orm.public.Invitation.where({ userId: inactive.id }).all()
    ).at(-1)!;
    assert.notEqual(invitation.tokenHash, secondToken);
    assert.equal(invitation.tokenHash.includes(secondToken), false);
    await publicRequest(
      `/auth/invitations/${secondToken}`,
      "POST",
    ...
    await publicRequest(
      "/auth/login",
      "POST",
      { email: inactive.email, password: "bad password" },
      401,
    );
    const login = await publicRequest("/auth/login", "POST", {
      email: inactive.email,
      password: "Initial password 123!",
    });
    const participantCookie = login.response.headers
      .getSetCookie()[0]!
      .split(";")[0]!;
    const participantAdmin = await fetch(base + "/admin/users", {
      headers: { Cookie: participantCookie },
    });
    assert.equal(participantAdmin.status, 403);
    const participantPractice = await fetch(base + "/practice", {
      headers: { Cookie: participantCookie },
    });
    ...
    const expiredInvite = await request(
      `/admin/users/${expired.id}/invitations`,
      "POST",
    );
    const expiredRecord = (
      await db.orm.public.Invitation.where({ userId: expired.id }).all()
    ).at(-1)!;
    await db.orm.public.Invitation.where({ id: expiredRecord.id }).update({
      expiresAt: new Date(Date.now() - 1000).toISOString(),
    });
    assert.equal(
      (
        await publicRequest(
          `/auth/invitations/${expiredInvite.url.split("/").at(-1)!}`,
        )
      ).data.valid,
      false,
    );

    const activeLogin = await publicRequest("/auth/login", "POST", {
      email: inactive.email,
      password: "Initial password 123!",
    });
    const activeCookie = activeLogin.response.headers
      .getSetCookie()[0]!
      .split(";")[0]!;
    const firstReset = await request(
      `/admin/users/${inactive.id}/password-resets`,
      "POST",
    );
    assert.ok(
      Math.abs(
        new Date(firstReset.expiresAt).getTime() -
          (Date.now() + 60 * 60 * 1000),
      ) < 10_000,
    );
    const secondReset = await request(
      `/admin/users/${inactive.id}/password-resets`,
      "POST",
    );
    assert.equal(
      (
        await publicRequest(
          `/auth/password-resets/${firstReset.url.split("/").at(-1)!}`,
        )
      ).data.valid,
      false,
    );
    ...
    await publicRequest(
      `/auth/password-resets/${resetToken}`,
      "POST",
    ...
    await publicRequest(
      "/auth/login",
      "POST",
      { email: inactive.email, password: "Initial password 123!" },
      401,
    );
    await publicRequest(
      "/auth/login",
      "POST",
      { email: inactive.email, password: "Changed password 123!" },
      200,
    );

    const resetExpiry = await request(
      `/admin/users/${inactive.id}/password-resets`,
      "POST",
    );
    const resetRecord = (
      await db.orm.public.PasswordReset.where({ userId: inactive.id }).all()
    ).at(-1)!;
    await db.orm.public.PasswordReset.where({ id: resetRecord.id }).update({
      expiresAt: new Date(Date.now() - 1000).toISOString(),
    });
    assert.equal(
      (
        await publicRequest(
          `/auth/password-resets/${resetExpiry.url.split("/").at(-1)!}`,
        )
      ).data.valid,
      false,
    );
